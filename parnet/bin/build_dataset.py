...
def build_tf_dataset(bed, dataspec, target_size, min_height):
    ...
    ds.filter(lambda example: tf.reduce_max(example['outputs']['eCLIP']) >= tf.constant(min_height, dtype=tf.float32))
...
def main(bed_train, bed_val, bed_test, dataspec, target_size, control, output_directory, min_height):
    # load dataspec
    dataspec = DataSpec(dataspec, control=control)

    split_datasets = {'train': build_tf_dataset(bed_train, dataspec, target_size, min_height)}
    if bed_val is not None:
        split_datasets['validation'] = build_tf_dataset(bed_val, dataspec, target_size, min_height)
    if bed_test is not None:
        split_datasets['test'] = build_tf_dataset(bed_test, dataspec, target_size, min_height)
